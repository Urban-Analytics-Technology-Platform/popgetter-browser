...
export const metrics = readable<{
  loading: boolean;
  value: Array<Metric> | null;
}>({ loading: true, value: null }, (set) => {
  fetch(
    "https://allofthedata.s3.us-west-2.amazonaws.com/acs/tracts_2019_fiveYear_variables.json",
  )
    .then((r) => r.json())
    .then((metrics) => {
...
export const metricTree = derived(metrics, ($metrics) => {
  if ($metrics.loading) {
    return $metrics;
  } else {
    let vals = $metrics.value;
    let paths = $metrics.value.map((v) => ({
      path: [v.universe, ...v.variableExtendedName.split("|")],
      id: v.uniqueID,
    }));
...
function createMetricValues() {
  ...
  const loadIds = (ids) => {
    ...
    if (ids && ids.length > 0) {
      ...
      fetch(`/api/census?metrics=${censusIds.join(",")}`)
        .then((r) => r.json())
        .then((data) => {
